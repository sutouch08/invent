...
<th style='width:15%;'>เลขที่เอกสาร</th><th>โซน</th><th style='width:10%; text-align:right;'>จำนวน</th><th style='width:15%; text-align:right;'>มูลค่า</th>
	...
	$sql = dbQuery("SELECT id_order_consignment, tbl_order_consignment.id_order, reference, tbl_order_consignment.id_customer, date_upd FROM tbl_order_consignment LEFT JOIN tbl_order ON tbl_order_consignment.id_order = tbl_order.id_order WHERE role = 5 AND ".$customer_query." AND (date_upd BETWEEN '$from' AND '$to' ) AND tbl_order.current_state IN(6,7,9)");
	$row = dbNumRows($sql);
	if($row>0){
		$i = 0;
		$o = 1;
...
if(isset($_GET['stock_by_warehouse'])&&isset($_GET['product'])){
	...
	if($r>0)
	{
		...
		{ 
			$sql = dbQuery("SELECT id_product_attribute FROM tbl_product_attribute WHERE id_product = '".$product."'"); 
			$row = dbNumRows($sql);
			$n = 1;
			$i = 0;
			while($i<$row)
