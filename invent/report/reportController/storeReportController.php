...
if( isset( $_GET['receive_product'] ) && isset( $_GET['report'] ) )
{
	...
	$t_rank	= $_POST['rank'];
	$data 	= array();
	if( $p_rank == 2 ){ $pq = "AND po_reference = '".$_POST['reference']."' "; }else{ 	$pq = "AND po_reference != '' "; }
	if( $s_rank == 2 ){ $sq = "AND id_supplier = ".$_POST['id_sup']." "; }else	{ $sq	= "AND id_supplier != '' "; }
	if(  $t_rank == 2 ){ $tq = " AND ( tbl_receive_product.date_add BETWEEN '".fromDate($_POST['from_date'], true)."' AND '".toDate($_POST['to_date'], true)."')"; }else{ $tq = ""; }
	$qs = "SELECT tbl_receive_product_detail.id_product, tbl_receive_product_detail.id_product_attribute, tbl_receive_product.reference, tbl_receive_product.id_po, tbl_po.id_supplier, ";
	...
	if( dbNumRows($qs) > 0 ) :
		...
		$total_amount 	= 0;
		while( $rs = dbFetchArray($qs) ) :
			$cost = get_product_cost_by_po($rs['id_po'], $rs['id_product_attribute']);
			$arr = array(
							"no" 				=> $n,
							"date_add" 		=> thaiDate($rs['date_add']),
							"product_code"	=> get_product_reference($rs['id_product_attribute']),							
							"reference" 		=> $rs['reference'],
							"po_reference" => $rs['po_reference'],
							"supplier"			=> supplier_name($rs['id_supplier']),
							"invoice"			=> $rs['invoice'],
							"qty" 				=> number_format($rs['qty'])	,
							"cost"				=> number_format($cost,2),
							"amount"			=> number_format($cost * $rs['qty'], 2)
						);
			array_push($data, $arr);		
			$total_qty += $rs['qty']; 
			$total_amount += $cost * $rs['qty'];
			$n++;
		endwhile;
		$arr = array(
...
if( isset( $_GET['receive_product'] ) && isset( $_GET['export'] ) )
{
	...
	$t_rank	= $_GET['rank'];
	$data 	= array();
	if( $p_rank == 2 ){ $pq = "AND po_reference = '".$_GET['reference']."' "; $p_title = $_GET['reference']; }else{ 	$pq = "AND po_reference != '' ";  $p_title = "ทั้งหมด"; }
	if( $s_rank == 2 ){ $sq = "AND id_supplier = ".$_GET['id_sup']." ";  $s_title = supplier_name($_GET['id_sup']); }else{ $sq	= "AND id_supplier != '' ";  $s_title = "ทั้งหมด";}
	if(  $t_rank == 2 )
	{ 
		$tq 		= " AND ( tbl_receive_product.date_add BETWEEN '".fromDate($_GET['from_date'], true)."' AND '".toDate($_GET['to_date'], true)."')"; 
		$t_from 	=  $_GET['from_date'];
		$t_to		= $_GET['to_date'];
	}
	else
	...
	$arr = array("รายงานการรับสินค้าเข้าจากการซื้อแสดงรายการสินค้า ");
	array_push($data, $arr);
	$arr = array("ใบสั่งซื้อ", $p_title, "ผู้ขาย", $s_title, "วันที่", $t_from, "ถึง", $t_to);
	array_push($data, $arr);
	$arr = array("********************************************************************************************************************");
	array_push($data, $arr);
	$arr = array("ลำดับ", "วันที่", "รหัส", "เลขที่เอกสาร", "ใบสั่งซื้อ", "ผู้ขาย", "เลขที่ใบส่งสินค้า", "ราคา", "จำนวน", "มูลค่า", "โซน");
	array_push($data, $arr);
	$qs = "SELECT tbl_receive_product_detail.id_product, tbl_receive_product_detail.id_product_attribute, tbl_receive_product.reference, tbl_receive_product.id_po, tbl_po.id_supplier, ";
	$qs .= "tbl_receive_product.po_reference, tbl_receive_product.invoice, tbl_receive_product.date_add, qty, tbl_receive_product_detail.id_zone ";
	$qs .= "FROM tbl_receive_product_detail JOIN tbl_receive_product ON tbl_receive_product_detail.id_receive_product = tbl_receive_product.id_receive_product ";
	...
	if( dbNumRows($qs) > 0 ) :
		...
		while( $rs = dbFetchArray($qs) ) :
			$cost = get_product_cost_by_po($rs['id_po'], $rs['id_product_attribute']);
			$arr = array( $n, thaiDate($rs['date_add']), get_product_reference($rs['id_product_attribute']), $rs['reference'], $rs['po_reference'], supplier_name($rs['id_supplier']), $rs['invoice'], $cost, $rs['qty'], $rs['qty'] * $cost, get_zone($rs['id_zone']) );
			array_push($data, $arr);	
			$total_qty	+= $rs['qty'];
			$total_amount	+= $cost * $rs['qty'];	
			$n++;
		endwhile;
		$arr = array( "", "", "", "", "", "","", "", $total_qty, $total_amount, "");
		array_push($data, $arr);
	else :
		$arr = array("ไม่มีข้อมูล");
		array_push($data, $arr);	
	...
	$excel 	= new Excel_XML("UTF-8", false, "received_product_detail");
	$excel->addArray($data);
	$excel->generateXML("received_product_detail");
	setToken($_GET['token']);
}
